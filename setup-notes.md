...
Flashed the OS onto a micro SD card using a card reader. Inserted the micro SD card into the slot available on Pi and installed the OS. To configure static IP we used nmcli. Faced some issues with DHCP reservation, static IP and VNC not being able to connect, all of which are documented in [troubleshooting.md].
