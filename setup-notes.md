...
Flashed the OS onto a micro SD card using a card reader. Inserted the micro SD card into the slot available on Pi and installed the OS. To configure static IP we used nmcli. Faced some issues with DHCP reservation, static IP and VNC not being able to connect, all of which are documented in [troubleshooting log](troubleshooting.md).
...
Filebeat is a lightweight log shipper that collects log files from Suricata, Zeek, and Cowrie running on the Raspberry Pi and forwards them to Elasticsearch on the main PC. Without Filebeat, the logs generated by the sensors on the Pi would stay on the Pi and never reach the SIEM for analysis. Allowed outgoing connections on the UFW firewall on port 9200 and incoming traffic for port 9200 on the main PC for filebeat. Installed Python 3 and supporting build tools required to install and run Cowrie in a later phase.
